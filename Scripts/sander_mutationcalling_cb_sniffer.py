		...
		with pysam.AlignmentFile(bam_fil, 'rb') as pile:
			...
			for pileupcolumn in pile.pileup(reference=self.chrm, start=self.start - 1, end=self.start,
				...
				for read in pileupcolumn.pileups:
					...
					if read.alignment.has_tag('CB') and read.alignment.has_tag('CB'):
						...
						qU_tag = read.alignment.get_tag('CB')
						...
						if indel > 0:
							...
							if self.alt == '-':
								...
								# 						1:read.query_position + read.indel + 1] == self.alt:
									...
									# UB
									altU_tag = read.alignment.get_tag('CB')
									...
									# UB
									refU_tag = read.alignment.get_tag('CB')
